...
class CaseClassesGenerator extends GeneratorForAnnotation<CaseClassAnnotation> {
    ...
    if (element.name.startsWith('_')) {
      ...
      if (constructors.length != 1) {
        throw InvalidGenerationSourceError(
            "@caseclass class must have exactly one constructor, but ${element.name} has ${constructors.length}");
      }

      final typeparams = element.listChildren<TypeParameterElement>().toList();

      final parameters = constructors[0]
          .listChildren<ParameterElement>()
          .map((e) => _ParameterInfo(e.name, e.type))
          .toList();
      ...
      res
        ...
        ..writeAll(
            parameters.map((p) =>
                "  final ${_maybeCaseClassName(p.dartType)}${_typeArguments(p.typeArgs.toList())} ${p.name}; // ${p.dartType is ParameterizedType}"),
            "\n")
        ..writeln()
        ..writeln()
        ..writeln("@protected")
        ...
        ..writeAll(
            parameters.map((p) => "      ${p.name} == other.${p.name}"), " && ")
        ..writeln(";")
        ..writeln("  }")
  ...
  String _maybeCaseClassName(DartType t) {
    if (TypeChecker.fromRuntime(CaseClassAnnotation)
        .hasAnnotationOfExact(t.element)) {
  ...
  String _typeParametersDeclaration(List<TypeParameterElement> elements) =>
      elements.isNotEmpty
          ? "<" +
              elements
                  .map((el) => el.bound == null
                      ? el.name
                      : "${el.name} extends ${el.bound}")
                  .join(", ") +
              ">"
          : "";

  String _typeArguments(List<DartType> types) => types.isNotEmpty
      ? "<" + types.map((t) => _maybeCaseClassName(t)).join(", ") + ">"
      : "";
