...
extension EitherSyntaxExt<L, A> on Either<L, A> {
  ...
  Either<L, B> map<B>(B mapper(A a)) {
    return EitherInstances<L>().functor.map(this, mapper).fix();
  }

  // applicative
  Either<L, B> ap<B>(Either<L, MapAction<A, B>> ff) {
    return EitherInstances<L>().applicative.ap(this, ff).fix();
  }

  // monad
  Either<L, B> flatMap<B>(Either<L, B> mapper(A a)) {
    return EitherInstances<L>().monad.flatMap(this, mapper).fix();
  }
  
  // show
  String show(Show<L> leftShow, Show<A> rightShow) {
    return EitherInstances<L>().show(leftShow, rightShow).show(this);
  }
}
